export const content = {
  // ================= LITERACY =================
  burmese_pyu: {
    category: "Birth of Literacy",
    title: "Burmese & Pyu Alphabet",
    subtitle: "Foundation of Literacy",
    description:
      "The Pyu script is one of the earliest writing systems used in ancient Myanmar by the Pyu civilization. It developed from the Brahmi script, which spread across South and Southeast Asia. The modern Burmese script is historically related because it also ultimately traces its origin to Brahmi. The Pyu script represents an early stage of writing culture in Myanmar and shares a common historical root with the Burmese alphabet.The Burmese script is the official writing system used to write the Burmese language in Myanmar. It is an abugida, meaning each consonant carries an inherent vowel sound that can be modified using different vowel marks. The script was developed around the 11th century during the Pagan Kingdom. The Burmese alphabet consists of 33 consonants and several vowel signs and diacritics, and its rounded shapes developed because it was traditionally written on palm leaves, where straight lines could tear the surface.",
    image: "https://www.skyknowledge.com/vengi-combined.jpg",
    detailed_image: "/images/Pyu_Burmese.jpg",
  },

  palm_throne: {
    category: "Birth of Literacy",
    title: "Palm Leaf Manuscript on Throne",
    subtitle: "Sacred Preservation",
    description:
      "A palm-leaf manuscript is a traditional form of writing used in Myanmar. Texts were carefully inscribed on dried palm leaves using a metal stylus and then preserved for religious, literary, and historical purposes. In Myanmar, many important Buddhist scriptures and teachings were recorded in this form. The manuscript is often placed on an ornate ceremonial stand or throne, symbolizing respect and reverence for sacred knowledge. These manuscripts played an important role in preserving traditional literature, religious teachings, and historical records before the introduction of modern printing.",
    image:
      "https://www.michaelbackmanltd.com/wp-content/archived-images/March_0602-964x329.jpg",
    detailed_images: [
      "/images/Palm-Leaf Manuscript on a throne.jpg",
      "/images/Palm-Leaf Manuscript on a throne1.jpg",
      "/images/Palm-Leaf Manuscript on a throne2.jpg",
    ],
  },

  myazedi: {
    category: "Birth of Literacy",
    title: "Myazedi Inscription (1113)",
    subtitle: "Rosetta Stone of Myanmar",
    description:
      "Rosetta Stone of Myanmar History”. Located in Bagan Historic City, the Myazedi quadrilingual stone inscription is the oldest Myanmar Language inscription documenting the history, religion, and culture in the 12th century A.D. The document is an inscription in four languages, Pyu, Mon, Myanmar, and Pali, on each of the four sides. Represents one of the earliest multilingual educational records in our history",
    image:
      "https://as1.ftcdn.net/jpg/04/25/32/82/1000_F_425328272_kQBdkeZBgvUHR9usVxU1CTfrppEiJHCt.jpg",
    detailed_image:
      "/images/Myazedi Quadrilingual Stone Inscription (1113).jpg",
  },

  buddhism_flags: {
    category: "Birth of Literacy",
    title: "Buddhism Flags",
    subtitle: "Spiritual Knowledge",
    description: `
The Buddhist flag is a modern symbol designed in the late 19th century to represent the unity of Buddhists and the universal nature of the Buddha's teachings. It consists of five vertical stripes followed by a sixth compound stripe on the fly.
<br>
<br>

Design and Symbolism:
<br>
<br>

<ul>
  <li>Blue (Nīla): Universal compassion.</li>
  <li>Yellow (Pīta): The Middle Way.</li>
  <li>Red (Lohita): Practice, wisdom, and virtue.</li>
  <li>White (Odāta): Purity and liberation.</li>
  <li>Orange (Mañjeṭṭha): Wisdom.</li>
  <li>Compound Stripe (Pabbhassara): A blend of the five colors representing universal truth.</li>
</ul>
`,

    image:
      "https://www.samyeinstitute.org/wp-content/uploads/2017/09/buddhist-flag-std.jpg",
    detailed_image:
      "https://khmerbuddhison.net/wp-content/uploads/2024/07/ea1dc-buddhist-flag-cambodia.jpg",
  },

  kuthodaw: {
    category: "Birth of Literacy",
    title: "Kuthodaw Pagoda",
    subtitle: "World’s Largest Book",
    description: `
The Kuthodaw Pagoda (Burmese: ကုသိုလ်တော်ဘုရား), formally known as Mahalawka Marazein, is a historic Buddhist stupa located at the foot of Mandalay Hill in Mandalay, Myanmar. It is famous worldwide for housing what is recognized as the "World's Largest Book".

Unlike a traditional book, it consists of 729 marble slabs, each inscribed on both sides with the Tipitaka, the complete Pali Canon of Theravada Buddhism. Each slab is preserved inside its own small white stupa, creating a remarkable complex of 729 shrines surrounding the central golden pagoda.

The pagoda was commissioned by King Mindon Min in 1857 to preserve Buddhist teachings for future generations. The inscriptions were completed and opened to the public on May 4, 1868.
`,
    image:
      "https://www.gomyanmartours.com/wp-content/uploads/2023/11/Kuthodaw-Pagoda-1.jpg",
    detailed_images: [
      "https://media-cdn.tripadvisor.com/media/photo-s/07/04/15/32/oriental-ballooning-private.jpg",
      "https://luxurymyanmarrivercruises.com/uploads/Kuthodaw-Pagoda-2.jpg",
      "https://media.istockphoto.com/id/1195760711/photo/golden-wide-angle-kuthodaw-pagoda-mandalay-hill-mandalay-myanmar.jpg?s=612x612&w=0&k=20&c=pc_tyE89II9VrsHs_mLoSFu5I-3T79fSQv7aW1GVtNw=",
    ],
  },

  university: {
    category: "Birth of Literacy",
    title: "University of Yangon",
    subtitle: "Higher Education",
    description:
      "The University of Yangon (formerly Rangoon University) is regarded as the premier and oldest institution of higher education in Myanmar, established in 1878 as Rangoon College and later established as a university in 1920. While it historically held a top-tier reputation in Southeast Asia, current global rankings place it differently based on modern assessment criteria.",
    image:
      "https://elevenmyanmar.com/sites/news-eleven.com/files/news-images/yu.jpg",
    detailed_images: [
      "https://developingtelecoms.com/images/stories/Features/800x600/Yangon_Technological_University_600.jpg",
      "https://media.licdn.com/dms/image/v2/C4D1BAQEhLKb8vZH50g/company-background_10000/company-background_10000/0/1583081675954?e=2147483647&v=beta&t=Fte9Cm7mOMP0Spqu7X8F31vmFpq6LbYVXpGum1Nukvc",
      "https://www.yangontimemachine.com/files/images/1024/convocation_1024_2.jpg",
    ],
  },

  colonial_map: {
    category: "Birth of Literacy",
    title: "Colonial Burma Map",
    subtitle: "Global Connection",
    description:
      "This historical map shows the extensive territories of British India during the colonial era, clearly illustrating that Burma (now Myanmar) was administered as part of the British Empire in the same way as India. In 1937, Burma was separated from British India to become a distinct crown colony, but during most of the colonial period shown on this map (especially maps from the late 19th to early 20th century), it was fully under British rule alongside India.",
    image:
      "https://upload.wikimedia.org/wikipedia/commons/d/dc/Map_of_Burma_%28Central_Section%29_from_The_Imperial_Gazetteer_of_India_%281907-1909%29.jpg",
    detailed_images: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSYJioAC_a_4eOYAEaYJgEgoAPeBAYMxT5yEQ&s",
      "https://thekootneeti.in/wp-content/uploads/2020/09/British_Indian_Empire_1909_Imperial_Gazetteer_of_India-1024x836.jpg",
      "https://pbs.twimg.com/media/FHmuLlsVEAEgbW0.jpg",
    ],
  },

  // ================= MONASTIC =================
  mingun: {
    category: "Monastic Education",
    title: "Mingun Bell",
    subtitle: "Heritage & Wisdom",
    description: `
<p>The Mingun Bell, located in Mingun, Myanmar, is widely celebrated as the largest uncracked, functioning bell in the world. Weighing approximately 90 metric tons (roughly 200,000 pounds), it stands about 12 feet high and measures over 16 feet in diameter at its base. 
.</p>

<br>
<br>

<ol>
  <li>Cast between 1808 and 1810 during King Bodawpaya’s reign</li>
  <li>Weighs about 90 tons (90,000 kg)</li>
  <li>Created for Mingun Pagoda near Mandalay</li>
  <li>One of the largest functioning (ringing) bells</li>
  <li>Symbolizes devotion to religion, culture, and knowledge</li>
  <li>Used to gather monks and the public for ceremonies</li>
  <li>Height: 3.7 meters (12 feet)</li>
  <li>Diameter: 5 meters (16.5 feet)</li>
  <li>Thickness: 30 cm (12 inches)</li>
</ol>
`,
    image:
      "https://41.media.tumblr.com/65b878aa3c39841ff278fc9e7e5a9d5f/tumblr_o28u3uBxhc1uijycio1_1280.png",
    detailed_images: [
      "https://img.atlasobscura.com/JOQhzYgAwNPD9h4wRgVo_1afGFfbR68L1O4pMyqG0XE/rs:fill:780:520:1/g:ce/q:81/sm:1/scp:1/ar:1/aHR0cHM6Ly9hdGxh/cy1kZXYuczMuYW1h/em9uYXdzLmNvbS91/cGxvYWRzL3BsYWNl/X2ltYWdlcy9iMWEz/ZTc0MjZkNThjNTk0/OGNlYjBiYjljYWU1/NDgyMzc3MmE4ZTIz/LmpwZw.jpg",
      "https://d122axpxm39woi.cloudfront.net/images/destinations/origin/6059a659ca41d.jpg",
      "https://as1.ftcdn.net/jpg/02/31/20/92/1000_F_231209220_jksKNy77gCH7RT0lC5GrxRdisLg7N3Cx.jpg",
      "https://www.gomyanmartours.com/wp-content/uploads/2014/11/Mingun-Bell.jpg",
    ],
  },

  monk_teaching: {
    category: "Monastic Education",
    title: "Monk Teaching",
    subtitle: "Traditional education in Myanmar before colonial rule",
    description:
      "Before colonial rule, traditional education in Myanmar was mainly conducted in Buddhist monasteries. Monks served as teachers and educated young boys in reading, writing, moral values, and religious teachings. Students learned the Buddhist scriptures, basic literacy, and traditional knowledge by studying palm-leaf manuscripts and memorizing texts. This monastic education system played an important role in spreading literacy and preserving culture and religious traditions across Myanmar before the arrival of British colonial rule in the 19th century.",
    image:
      "https://img.freepik.com/premium-photo/monks-convey-teach-dharma-novices-ancient-temples-phra-nakhon-si-ayutthaya-thailand_369024-802.jpg",
    detailed_images: [
      "https://www.myanmaritv.com/sites/default/files/styles/news_detail_image/public/vlcsnap-2022-06-08-14h50m39s38.jpg?itok=4EjVXZoh",
      "https://cdn.myportfolio.com/38afc973-b4fa-423c-b89c-d702a3ec3c75/7b9cc9b8-d75d-4649-8ed4-2f20c9a17898_rw_1920.jpg?h=72f6997344abe10ee6e465f53588f56d",
      "https://upload.wikimedia.org/wikipedia/commons/6/6d/Monk_examinations%2C_Bago%2C_Myanmar.jpg",
      "https://www.semesteratsea.org/wp-content/uploads/2022/09/Photo-12.jpg",
    ],
  },

  manuscript_tools: {
    category: "Monastic Education",
    title: "Ink pot & Stylus",
    subtitle: "Learning Tools",
    description: `
This image shows traditional tools used for creating palm-leaf manuscripts (pe-sa) in Myanmar. These tools were essential for preserving religious texts, literature, and historical records.
`,

    points: [
      "The set includes an iron stylus (kanyit) and a small decorated ink pot made of bronze or brass.",
      "The stylus is a slender, pointed tool about 15–20 cm long, used to carve text into palm leaves.",
      "Scribes held the stylus in the right hand while steadying the leaf with the left thumb.",
      "A small notch in the thumbnail was often used to guide the stylus and prevent slipping.",
      "Instead of writing with ink, characters were incised (scratched) directly onto the leaf surface.",
      "This method was commonly used for Buddhist scriptures, commentaries, and other texts.",
      "The rounded shape of Burmese script developed because sharp angles could damage the leaf.",
      "After carving, soot-based ink was applied to fill the grooves and make the text visible.",
      "Excess ink was wiped away, leaving dark, readable characters inside the incised lines.",
      "The ink pot stored thick black ink made from lampblack, oil, and natural resins.",
      "These tools were part of a monk’s or scribe’s personal kit used for writing and preservation.",
    ],
    image:
      "https://1.bp.blogspot.com/-hbB1ZLV-sfM/TvHSvfiEdKI/AAAAAAAAABc/hlUsHSwGc2c/s1600/inkpot+4.jpg",
    detailed_images: [
      "https://m.media-amazon.com/images/I/81mflie5J2L.jpg",
      "https://m.media-amazon.com/images/I/71sO2tp9mcL.jpg",
      "https://5.imimg.com/data5/SELLER/Default/2024/6/429175300/AT/TE/RH/111270851/vintage-feather-pen-and-ink-pot-set-500x500.jpg",
    ],
  },

  tipitaka: {
    category: "Monastic Education",
    title: "Tipitaka on the Offering Stand",
    subtitle: "Sacred Text",
    description:
      "A sacred display of the Tipiṭaka in palm-leaf manuscript form, placed on an ornate altar with offerings, representing devotion, learning, and the preservation of Buddhist teachings in Myanmar.",
    points: [
      "Depicts the Tipiṭaka (Three Baskets of the Pali Canon) in traditional palm-leaf manuscript form (pe-sa).",
      "The manuscripts are placed on an ornate offering stand or altar platform used in Buddhist worship.",
      "Palm-leaf bundles are tied with cords and protected by decorated wooden covers with gold inscriptions.",
      "The covers often include Burmese script and floral designs, symbolizing sacred value and respect.",
      "The manuscripts contain teachings from the Vinaya, Sutta, and Abhidhamma Piṭaka.",
      "Texts are written in Pali using the traditional Burmese rounded script.",
      "The display is elevated on a tiered stand, often draped with orange cloth symbolizing monastic robes.",
      "A white cloth featuring the Dhammacakka (Wheel of Dhamma) represents Buddhist teachings.",
      "Offerings such as jasmine flowers, incense bowls, and candles are placed alongside the texts.",
      "This arrangement is common in homes, monasteries, and pagodas for daily or ritual worship (pūjā).",
      "The Tipiṭaka is honored as part of the Three Jewels (Buddha, Dhamma, Sangha).",
      "It reflects the tradition of preserving sacred teachings across generations in Myanmar.",
      "Connected to historical events like the Fifth Buddhist Council under King Mindon.",
      "Palm-leaf manuscripts remain symbolic even in the modern era of printed texts.",
    ],
    image: "/images/Tipitaka.jpg",
    detailed_images: [
      "https://www.upf.edu/documents/232286848/232891487/MsFM7_10.jpg/d06e6c59-fe7c-4548-3c60-f5aca64f6d35?t=1583241936000",
      "https://upload.wikimedia.org/wikipedia/commons/f/f2/Tipitaka_scripture.jpg",
      "/images/Tipitaka.jpg",
    ],
  },

  // ================= COLONIAL =================
  colonial_classroom: {
    category: "Colonial Education",
    title: "Colonial Classroom",
    subtitle: "Formal Schooling",
    description:
      "The British administration built upon Burma's already impressive traditional foundation by establishing a widespread secular education system that emphasized practical, useful knowledge suited to modern life. By the late 19th and early 20th centuries, thousands of government and aided lay schools opened across the country, making formal education more accessible in urban and semi-urban areas. Subjects like English, arithmetic, geography, and basic sciences provided students with skills for administration, commerce, and professional roles, fostering a new class of educated Burmese capable of engaging with the wider world.",
    image:
      "https://commons.trincoll.edu/edreform/files/2014/05/CORNELL_ECHOLS_1039410872.jpg",
    detailed_images: [
      "https://www.hrw.org/sites/default/files/styles/16x9_large/public/multimedia_images_2019/201911crd_rohingya_education_still001.jpg?itok=ZYPYw5_M",
      "https://www.newmandala.org/wp-content/uploads/cache/2019/12/NEWMANDELA-1/1533149811.jpg",
    ],
  },

  // ================= MODERN =================
  modern_system: {
    category: "Post-Independence",
    title: "Slate Tablet and Pencil",
    subtitle: "Growth of Learning",
    description:
      "This image shows a traditional slate board and slate pencil used in Burmese classrooms, allowing students to practice writing and drawing in a simple, reusable, and cost-effective way.",
    points: [
      "A classic slate tablet paired with a slate pencil used in traditional Burmese education.",
      "Commonly used during the British colonial era and into the mid-20th century.",
      "Made of a smooth dark slate surface with a wooden frame for durability.",
      "Students wrote using a soft slate pencil that produced white or gray marks.",
      "Used for practicing writing, arithmetic, drawing, and spelling.",
      "The surface could be easily cleaned using cloth, water, or natural materials.",
      "Reusable design made it economical and environmentally friendly.",
      "Widely used in schools where resources were limited.",
      "Represents early classroom learning methods in Myanmar.",
    ],
    image:
      "https://www.townsends.us/cdn/shop/products/s3168IMG_0853_600x.jpg?v=1711979679",

    detailed_images: [
      "/images/slate.png",
      "https://t3.ftcdn.net/jpg/06/85/69/46/360_F_685694671_AOXjFbVZ5cRyUOXbfwaUPFmzMHVFW5jZ.jpg",
      "https://m.media-amazon.com/images/I/612doC-yXvL.jpg",
    ],
  },

  morality: {
    category: "Post-Independence",
    title: "Morality & Discipline",
    subtitle: "Core Values",
    description:
      "This image represents two important flags in Myanmar: the 1948 Independence Flag symbolizing ethnic unity, and the Buddhist Flag representing spiritual values through five sacred colors.",
    points: [
      "The 1948 Independence Flag of Burma featured one large white star surrounded by five smaller stars.",
      "The five small stars represented unity among major ethnic groups: Burman, Karen, Shan, Kachin, and Chin.",
      "The flag had a red background with a blue canton in the top-left corner.",
      "It was the national flag from 1948 to 1974 and is still a symbol of democracy.",
      "The Buddhist Flag (Sasana Flag) is widely used across Myanmar.",
      "It features five main colors: blue, yellow, red, white, and orange (sometimes pink).",
      "These colors represent the aura of the Buddha at enlightenment.",
      "Blue symbolizes compassion and kindness.",
      "Yellow represents the Middle Way.",
      "Red stands for practice and spiritual strength.",
      "White represents purity and truth of the Dhamma.",
      "Orange (or pink) symbolizes wisdom and teachings of the Buddha.",
    ],
    image:
      "https://flagsdb.com/upload/media/default/0001/01/6ef1a42786fd0319676ef53d6a8a4dc1cf0076c8.png",
    detailed_images: [
      "/images/Myanmar.jpg",
      "https://flagsdb.com/upload/media/default/0001/01/6ef1a42786fd0319676ef53d6a8a4dc1cf0076c8.png",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSIy_pmEXptMjDAK9RDOhEHY7c8N6UxuMbOjw&s",
    ],
  },

  education: {
    category: "Post-Independence",
    title: "School Uniforms for boys and girls",
    subtitle: "Foundation",
    description:
      "The Myanmar 'White-Green' school uniform is a standardized dress worn by students, symbolizing purity, calmness, equality, and national identity.",
    points: [
      "Known as the 'White-Green' uniform, worn in public schools across Myanmar.",
      "Mandatory for students from kindergarten to high school.",
      "Introduced as a standard dress code in 1966.",
      "White color represents purity and discipline.",
      "Green color symbolizes calmness and peace.",
      "Promotes equality by reducing visible social and economic differences.",
      "Creates a sense of unity among students.",
      "Reflects cultural values of modesty and simplicity.",
      "Represents national identity and shared educational experience.",
    ],
    image:
      "https://media.licdn.com/dms/image/v2/C5612AQHCobR6xsb6dw/article-cover_image-shrink_720_1280/article-cover_image-shrink_720_1280/0/1588661285153?e=2147483647&v=beta&t=VCLtPw_4-m8K7MPPEhouS9cM5fZt_XBo1yDnrDO1iXA",
    detailed_images: [
      "https://media.istockphoto.com/id/143176419/photo/lets-go-to-school.jpg?s=612x612&w=0&k=20&c=7pFJx141uBiz6n2T1K_FV_hIRDrYpDOnzmp7hiUlCAg=",
      "https://img2.chinadaily.com.cn/images/202001/16/5e1fbcdea3101282064c533b.jpeg",
      "https://www.globaltimes.cn/Portals/0/attachment/2011/98943dd8-a55e-492a-8ff4-b1aedeaf958e.jpg",
    ],
  },

  // ================= AI =================
  ai_brain: {
    category: "AI Learning",
    title: "Young Innovators Of Myanmar",
    subtitle: "Future Learning",
    description:
      "A Myanmar student team won first prize at an international robotics competition with an AI-powered search and rescue robot designed for disaster response.",
    points: [
      "Team 'We Are Genius Myanmar' won first prize at the World Robot Olympiad Asia Pacific Open Championship.",
      "The competition was held in the Philippines from 19 to 21 September.",
      "The team developed an AI-powered search and rescue robot vehicle.",
      "The robot uses autonomous navigation to operate without human control.",
      "Designed to enter disaster-affected areas where human access is difficult.",
      "Helps in search and rescue operations during natural disasters.",
      "Demonstrates innovation and practical use of AI in real-world situations.",
      "Team members include Moe Pyae Sone Kyaw (11), Kyaw Bhone Khant Zaw (11), and Nyan Koe (12).",
      "Represents the growing potential of young innovators in Myanmar.",
      "Highlights Myanmar’s progress in robotics and AI education.",
    ],
    image: "/images/young1.png",
    detailed_images: ["/images/young1.png", "/images/young2-2.png"],
  },

  online_learning: {
    category: "AI Learning",
    title: "Online Learning",
    subtitle: "Global Access",
    description:
      "Myanmar is embracing artificial intelligence as a tool for innovation, development, and inclusive growth, supported by national strategies and regional collaboration.",
    points: [
      "Myanmar is adopting AI to drive innovation and national development.",
      "AI is seen as a key tool for inclusive growth and future opportunities.",
      "The Ministry of Science and Technology is leading AI strategy development.",
      "A national AI policy is being drafted to guide implementation.",
      "Focus areas include education, development sectors, and public awareness.",
      "Promotes ethical and responsible use of artificial intelligence.",
      "Emphasis on building skilled human resources in AI.",
      "Encourages international cooperation and partnerships.",
      "Aligns with ASEAN frameworks on AI ethics and governance.",
      "Supports sustainable and long-term AI ecosystem growth in Myanmar.",
    ],
    image: "https://www.ilbcedu.com/wp-content/uploads/2018/07/pre-com.jpg",
    detailed_images: [
      "https://www.globalgiving.org/pfil/43162/pict_large.jpg",
      "https://www.infoans.org/media/k2/items/cache/e4f8cbf52d42e2ebd0b31e2bb395c29a_XL.jpg",
      "https://www.mdn.gov.mm/sites/default/files/articles/Free%20computer%20course%20opens%20in%20Naungcho%20Tsp..jpg",
    ],
  },

  robotics: {
    category: "AI Learning",
    title: "Robotics",
    subtitle: "Innovation",
    description:
      "Students in Myanmar participate in the World Robot Olympiad 2024, showcasing their skills in robotics, innovation, and teamwork.",
    points: [
      "Students are actively building and programming robots during the competition.",
      "Part of the World Robot Olympiad (WRO) 2024 held in Yangon, Myanmar.",
      "Encourages innovation, creativity, and problem-solving skills.",
      "Provides hands-on experience in robotics and artificial intelligence.",
      "Promotes teamwork and collaboration among young learners.",
      "Helps students apply technical knowledge in real-world challenges.",
      "Supports the growth of STEM education in Myanmar.",
      "Highlights increasing interest in robotics among youth.",
      "Prepares students for future careers in technology and engineering.",
    ],
    image:
      "https://www.mdn.gov.mm/sites/default/files/articles/KHP_0005-72-1024x576.jpg",
    detailed_images: ["/images/young2.png", "/images/young3.png"],
  },

  // ================= Leader =================
  Mingun_Sayadaw: {
    category: "Influential Personalities",
    title: "Mingun Sayadaw",
    subtitle: "Spiritual Knowledge & Extraordinary Memory",
    highlight: "Guinness World Record holder for human memory",

    description:
      "Mingun Sayadaw, also known as Bhaddanta Vicittasara, was one of Myanmar’s most respected Buddhist monks, renowned for his extraordinary memory and deep knowledge of the Tipiṭaka. He became internationally recognized for his ability to recite extensive Buddhist scriptures from memory, symbolizing the preservation of spiritual knowledge.",

    points: [
      "Recognized by Guinness World Records in 1985 for exceptional human memory.",
      "Successfully recited around 16,000 pages of Buddhist canonical texts from memory.",
      "Gained recognition during a major recitation event in Rangoon (Yangon) in 1954.",
      "Known for rare eidetic memory — the ability to recall information with high precision.",
      "Played an important role in preserving the Tipiṭaka (Buddhist scriptures).",
      "His achievements highlight Myanmar’s strong tradition of monastic education.",
      "Respected as a symbol of wisdom, discipline, and dedication to learning.",
      "Inspired generations of monks and scholars in Myanmar and beyond.",
      "Represents the deep connection between spirituality and knowledge in Myanmar culture.",
    ],

    image:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRofOyJ8uGqsM3S6qBLHtNQEngneWniO-Uo7g&s",

    detailed_images: [
      "https://i.pinimg.com/736x/a0/49/e8/a049e8a443d703245cf5fc38891d4ac1.jpg",
      "https://media-cdn.tripadvisor.com/media/photo-p/12/bc/13/b0/mingun-sayadaw.jpg",
    ],
  },

  General_Aung_San: {
    category: "Influential Personalities",
    title: "General Aung San",
    subtitle: "Father of Independence",

    description:
      "General Aung San is regarded as the founding father of modern Myanmar, leading the country toward independence from British rule.",

    points: [
      "Key leader in Myanmar’s independence movement.",
      "Founder of the modern Myanmar Armed Forces.",
      "Played a major role in negotiating independence from Britain.",
      "Assassinated in 1947 before independence was achieved.",
      "Remembered as a national hero and symbol of unity.",
      "His legacy continues to influence Myanmar’s history and identity.",
    ],

    image:
      "https://i0.wp.com/rohingyakhobor.com/wp-content/uploads/2024/08/Aung-San.jpg?fit=286%2C176&ssl=1",
    detailed_images: [
      "https://upload.wikimedia.org/wikipedia/commons/b/bc/Aung_San_color_portrait.jpg",
      "https://media.cnn.com/api/v1/images/stellar/prod/210201135601-02-aung-san-suu-kyi-gallery-restricted.jpg?q=w_1576,c_fill",
      "https://www.lowyinstitute.org/sites/default/files/General_Aung_San_cropped.jpg",
    ],
  },

  Nine_Martyrs: {
    category: "Influential Personalities",
    title: "Nine Martyrs",
    subtitle: "Heroes of Independence",

    description:
      "The Nine Martyrs were national leaders who were assassinated alongside General Aung San in 1947 while working toward Myanmar’s independence.",

    points: [
      "Killed on 19 July 1947 during a cabinet meeting.",
      "Played key roles in the independence movement.",
      "Honored annually on Martyrs’ Day in Myanmar.",
      "Represent sacrifice for national freedom.",
      "Symbolize courage, unity, and patriotism.",
      "Remain deeply respected figures in Myanmar history.",
    ],

    image:
      "https://media-cdn.tripadvisor.com/media/photo-s/0c/b4/90/9f/martyrs-mausoleum-at.jpg",
    detailed_images: [
      "https://www.mdn.gov.mm/sites/default/files/articles/Preserving.jpg",
      "https://media-cdn.tripadvisor.com/media/photo-s/0b/80/93/f9/caption.jpg",
      "https://www.myanmars.net/images/martyrs_mausoleum.jpg",
    ],

    Aung_San_Suu_Kyi: {
      category: "Influential Personalities",
      title: "Daw Aung San Suu Kyi",
      subtitle: "Peace & Democracy Leader",

      description:
        "Daw Aung San Suu Kyi is an internationally recognized leader known for her non-violent struggle for democracy and human rights in Myanmar.",

      points: [
        "Awarded the Nobel Peace Prize in 1991.",
        "Leader of Myanmar’s pro-democracy movement.",
        "Spent years under house arrest for her activism.",
        "Received the Presidential Medal of Freedom.",
        "Advocated peaceful resistance and human rights.",
        "A globally recognized political and social figure.",
      ],

      image: "/images/suu-kyi.jpg",
      detailed_images: ["/images/suu-kyi.jpg"],
    },
  },
  Aung_San_Suu_Kyi: {
    category: "Influential Personalities",
    title: "Daw Aung San Suu Kyi",
    subtitle: "Peace & Democracy Leader",

    description:
      "Daw Aung San Suu Kyi is an internationally recognized leader known for her non-violent struggle for democracy and human rights in Myanmar.",

    points: [
      "Awarded the Nobel Peace Prize in 1991.",
      "Leader of Myanmar’s pro-democracy movement.",
      "Spent years under house arrest for her activism.",
      "Received the Presidential Medal of Freedom.",
      "Advocated peaceful resistance and human rights.",
      "A globally recognized political and social figure.",
    ],

    image: "https://diplomacybeyond.com/wp-content/uploads/2021/02/image-1.jpg",
    detailed_images: [
      "https://static.independent.co.uk/s3fs-public/thumbnails/image/2018/03/28/09/win-myint.jpg",
      "https://cdn.theatlantic.com/thumbor/CgTAP9WCn7a9AhL2WPxnPKufazc=/0x1:1996x1997/1080x1080/media/img/mt/2018/09/RTX5T1H9/original.jpg",
      "https://cdn.jagonews24.com/media/imgAllNew/BG/2016May/Hasina-Su20160920115200.jpg",
      "https://i1.wp.com/asiatimes.com/wp-content/uploads/2018/10/India-Myanmar-Narendra-Modi-Aung-San-Suu-Kyi-2018-e1560326028950.jpg?fit=1600%2C1012&ssl=1",
    ],
  },

  U_Thant: {
    category: "Influential Personalities",
    title: "U Thant",
    subtitle: "Global Diplomat & UN Leader",

    description:
      "U Thant was a Myanmar diplomat who served as the Secretary-General of the United Nations, becoming the first Asian to hold the position.",

    points: [
      "Served as UN Secretary-General from 1961 to 1971.",
      "First Asian and non-Scandinavian to hold the position.",
      "Played a key role during global crises like the Cuban Missile Crisis.",
      "Promoted peace, diplomacy, and international cooperation.",
      "Served for over 10 years, one of the longest tenures.",
      "Highly respected global leader from Myanmar.",
    ],

    image:
      "https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEgLr7Sm50Jax-6EIhf3-zyl0-l2rFwPPCaOxB0ncPT7pYB0LbQ_CxfhCsW2m_K0InJEHvMU_bBcVIbQRUkuM-40sk9kCd1S-oOPsyE3yDGYGRH_bHv_SMIRw_6UGE9VmH-Owk6VUhrrC5Y/s1600/U+Thant.jpg",
    detailed_images: [
      "https://www.the-tls.com/wp-content/uploads/2025/11/u-thant-1970-1040791552.jpg",
      "https://news.un.org/en/sites/news.un.org.en/files/legacy-news-images/photos/large/2017/January/239122-B25.jpg",
      "https://bidkit.ams3.digitaloceanspaces.com/23/imgBig/141/173.jpg",
    ],
  },

  U_Kyi: {
    category: "Influential Personalities",
    title: "U Kyi",
    subtitle: "Early Robotics Pioneer",

    description:
      "U Kyi is recognized as an early innovator in robotics in Asia, contributing to technological advancement and inspiring future generations.",

    points: [
      "Credited with developing one of the first robots in Asia.",
      "Pioneer in early technological innovation in Myanmar.",
      "Inspired interest in robotics and engineering.",
      "Represents the beginning of modern technological development.",
      "Symbol of innovation and forward-thinking mindset.",
      "Encourages future generations in science and technology.",
    ],

    image:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSwrZZyJqrAmKXmqe--E6Jgh-1Zk1QwCdnZ2A&s",
    detailed_images: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTnKSZ4H84gnwHrnfNqOPLisvID1qQiJjBBuQ&s",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS_ZKshAQEmBttRRtbjGnapzKF-JclBI4R5Tw&s",
    ],
  },
};
